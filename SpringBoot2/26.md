...
Shiro 是 Apache
```
旗下开源的一款强大且易用的Java安全框架，身份验证、授权、加密、会话管理。** 相比 `Spring Security` 而言 `Shiro` 更加轻量级，且 API 更易于理解…
...
启动 `Chapter25Application.java` 中的 `main` 方法，为了更好的演示效果这里打开了 `postman` 做的测试，只演示其中一个流程，剩下的可以自己复制代码测试… 先登录，由于 `u3` 在 `DBCache` 中拥有的角色是 `test`，只有 `user:list` 这一个权限 ![登录](https://gitee.com/hezhiyuan007/java-study/raw/master/images/SpringBoot2/e121a4f9-183f-4482-9cb2-b20f392a10d9.png) 访问 `/users/query` 成功，因为我们符合响应的角色/权限 ![访问Query接口](https://gitee.com/hezhiyuan007/java-study/raw/master/images/SpringBoot2/671be63c-440a-41ca-96cc-a030bcb98cc5.png) 访问 `/users/find` 失败，并重定向到了 `/denied` 接口，问题来了为什么 `/users/find` 没有写注解也权限不足呢？ ![权限不足](https://gitee.com/hezhiyuan007/java-study/raw/master/images/SpringBoot2/2df93108-3edb-4689-91d9-493ae6540055.png) 细心的朋友肯定会发现 **在 ShiroConfiguration 中写了一句 permissions.put(“/users/find”, “perms[user:find]”);** 意味着我们不仅可以通过注解方式，同样可以通过初始化时加载数据库中的权限树做控制，看各位喜好了….

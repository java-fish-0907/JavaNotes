...
- 时间复杂度：𝑂(1)O(1)，“出栈”、“入栈”、“查看栈顶元素”的操作不论数据规模多大，都只是有限个步骤，因此时间复杂度是：𝑂(1)O(1)。
- 空间复杂度：𝑂(𝑁)O(N)，这里 𝑁N 是读出的数据的个数。
...
- 时间复杂度：𝑂(1)O(1)，“出栈”、“入栈”、“查看栈顶元素”的操作不论数据规模多大，都只有有限个步骤，因此时间复杂度是：𝑂(1)O(1)。
- 空间复杂度：𝑂(𝑁)O(N)，这里 𝑁N 是读出的数据的个数。

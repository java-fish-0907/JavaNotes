![](https://gitee.com/hezhiyuan007/java-study/raw/master/images/OperatingSystem/f01f226d-b8dd-4b6c-89ae-39d35e3dc606.png)
...
![](https://gitee.com/hezhiyuan007/java-study/raw/master/images/OperatingSystem/8f4ceff9-4399-4f15-889f-b043a8a3a584.png)

## 正文

先来认识 CPU 的架构，只有理解了 CPU 的 架构，才能更好地理解 CPU 是如何读写数据的，对于现代 CPU 的架构图如下：

![](https://gitee.com/hezhiyuan007/java-study/raw/master/images/OperatingSystem/4446cf65-2e36-4a5f-a350-65f03058ebc2.png)
...
![](https://gitee.com/hezhiyuan007/java-study/raw/master/images/OperatingSystem/991633b0-acd3-4888-a4b6-aa985625c509.png)

从上图也可以看到，从上往下，存储设备的容量会越大，而访问速度会越慢。至于每个存储设备的访问延时，你可以看下图的表格：

![](https://gitee.com/hezhiyuan007/java-study/raw/master/images/OperatingSystem/3adc1b27-ce6d-459b-9d88-3c5be901b8f9.png)
...
![](https://gitee.com/hezhiyuan007/java-study/raw/master/images/OperatingSystem/0805a2bd-5b24-47b0-a9c3-926cf6914b4a.png)
...
![](https://gitee.com/hezhiyuan007/java-study/raw/master/images/OperatingSystem/57ae5107-96e0-410c-8184-2b3069ce06a4.png)
...
![](https://gitee.com/hezhiyuan007/java-study/raw/master/images/OperatingSystem/3960a816-cad8-4f2f-abb8-220d73e7b040.png)

②. 1 号核心读取变量 A，由于 CPU 从内存读取数据到 Cache 的单位是 Cache Line，也正好变量 A 和 变量 B 的数据归属于同一个 Cache Line，所以 A 和 B 的数据都会被加载到 Cache，并将此 Cache Line 标记为「独占」状态。

![](https://gitee.com/hezhiyuan007/java-study/raw/master/images/OperatingSystem/25d44555-47ce-49f6-bf08-99a72ad4760d.png)
...
![](https://gitee.com/hezhiyuan007/java-study/raw/master/images/OperatingSystem/58861ae9-e99c-41a0-9796-5a78a8f3e059.png)

④. 1 号核心需要修改变量 A，发现此 Cache Line 的状态是「共享」状态，所以先需要通过总线发送消息给 2 号核心，通知 2 号核心把 Cache 中对应的 Cache Line 标记为「已失效」状态，然后 1 号核心对应的 Cache Line 状态变成「已修改」状态，并且修改变量 A。

![](https://gitee.com/hezhiyuan007/java-study/raw/master/images/OperatingSystem/b2eee4b3-e468-471e-9c38-666789bbbfab.png)

⑤. 之后，2 号核心需要修改变量 B，此时 2 号核心的 Cache 中对应的 Cache Line 是已失效状态，另外由于 1 号核心的 Cache 也有此相同的数据，且状态为「已修改」状态，所以要先把 1 号核心的 Cache 对应的 Cache Line 写回到内存，然后 2 号核心再从内存读取 Cache Line 大小的数据到 Cache 中，最后把变量 B 修改到 2 号核心的 Cache 中，并将状态标记为「已修改」状态。

![](https://gitee.com/hezhiyuan007/java-study/raw/master/images/OperatingSystem/767947ac-5bb8-4d5d-bff8-c350f9971b1e.png)
...
![](https://gitee.com/hezhiyuan007/java-study/raw/master/images/OperatingSystem/f3f70b6c-af30-4249-8e7a-fc941fecc3db.png)
...
![](https://gitee.com/hezhiyuan007/java-study/raw/master/images/OperatingSystem/63e6ca87-21ab-49bb-8c01-248e69131e78.png)

结构体里的两个成员变量 a 和 b 在物理内存地址上是连续的，于是它们可能会位于同一个 Cache Line 中，如下图：

![](https://gitee.com/hezhiyuan007/java-study/raw/master/images/OperatingSystem/d57a261a-cb98-477f-98d9-d87fe6224aba.png)

所以，为了防止前面提到的 Cache 伪共享问题，我们可以使用上面介绍的宏定义，将 b 的地址设置为 Cache Line 对齐地址，如下：

![](https://gitee.com/hezhiyuan007/java-study/raw/master/images/OperatingSystem/b2ce3774-6ac3-4e85-bf39-c6101b72e5a7.png)

这样 a 和 b 变量就不会在同一个 Cache Line 中了，如下图：

![](https://gitee.com/hezhiyuan007/java-study/raw/master/images/OperatingSystem/faa90690-0fec-4e88-8c4c-2cef0311c267.png)
...
![](https://gitee.com/hezhiyuan007/java-study/raw/master/images/OperatingSystem/fbe44f69-b0f5-4c24-a252-73e007f2268d.png)
...
![](https://gitee.com/hezhiyuan007/java-study/raw/master/images/OperatingSystem/709884cc-3a59-4ee6-bc5c-40fdafc25209.png)
...
![](https://gitee.com/hezhiyuan007/java-study/raw/master/images/OperatingSystem/afe957ce-d41f-43e1-b5ad-bd69ef4adda5.png)
...
![](https://gitee.com/hezhiyuan007/java-study/raw/master/images/OperatingSystem/fb102fa9-3369-4b29-a08e-fa1379177f28.png)
...
![](https://gitee.com/hezhiyuan007/java-study/raw/master/images/OperatingSystem/73d95199-3c6a-4d28-9d87-318b5913869a.png)
...
![](https://gitee.com/hezhiyuan007/java-study/raw/master/images/OperatingSystem/34f231b2-dc0c-47ab-8166-3979eed95c9f.png)
...
![](https://gitee.com/hezhiyuan007/java-study/raw/master/images/OperatingSystem/50b4acd0-0caf-497d-bd87-f3b3f2c58c27.png)
...
![](https://gitee.com/hezhiyuan007/java-study/raw/master/images/OperatingSystem/79ae63ef-5cd8-4588-bcc8-6bb5d130164e.png)

如果想修改已经运行中的任务的优先级，则可以使用 `renice` 来调整 nice 值：

![](https://gitee.com/hezhiyuan007/java-study/raw/master/images/OperatingSystem/8ce13670-b457-4ec3-abf1-00a9ef0a64aa.png)

nice 调整的是普通任务的优先级，所以不管怎么缩小 nice 值，任务永远都是普通任务，如果某些任务要求实时性比较高，那么你可以考虑改变任务的优先级以及调度策略，使得它变成实时任务，比如：

![](https://gitee.com/hezhiyuan007/java-study/raw/master/images/OperatingSystem/db0ee9f9-9377-4e92-a6b7-673a7ae31868.png)
...
[知道硬盘很慢，但没想到比 CPU Cache 慢 10000000 倍](https://mp.weixin.qq.com/s/-E5jcp7tfkXjsSu2vzdeAw)

[如何写出让 CPU 跑得更快的代码？](https://mp.weixin.qq.com/s/-uhAhBD2zGl_h19E4fNJzQ)

[10 张图打开 CPU 缓存一致性的大门](https://mp.weixin.qq.com/s/PDUqwAIaUxNkbjvRfovaCg)

哈喽，我是小林，微信搜索「**小林coding**」，这里有更多的图解计算机基础文章。

最后，如果觉得文章对你有帮助，欢迎分享给你的朋友，也给小林点个「点+收藏」，这对小林非常重要，谢谢你们，给各位小姐姐小哥哥们抱拳了，我们下次见！
